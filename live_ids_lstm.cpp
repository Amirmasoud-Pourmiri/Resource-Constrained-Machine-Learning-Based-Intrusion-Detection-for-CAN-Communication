// ids_live_labeled_pycompat.cpp
// Live CAN reader (socketCAN) -> 10-frame sliding windows -> MinMax scaling
// -> DOT decision tree inference. No labels or accuracy computation, but adapted
// for named features in DOT (like t9:Timestamp) from the labeled version.
//
// Build: g++ -O2 -std=c++17 ids_live_labeled_pycompat.cpp -o ids_live_labeled
// Run:   sudo ./ids_live_labeled --live can0
//
// Needs: lstm_pruned_tree.dot (exported/pruned tree)

#include <algorithm>
#include <array>
#include <cctype>
#include <cstdio>      // for perror and std::snprintf
...
static const char*    DOT_FILE = "lstm_pruned_tree.dot";
...
/* ───────── DOT parser (adapted from labeled version for named features) ───────── */
Tree parse_dot(const std::string& path, int& root_idx, int& max_feat_idx){
    ...
    // Use custom delimiter for raw string to avoid issues
    std::regex rg_node(R"regex(^\s*(\d+)\s+\[label="([^<]+)\s+<=\s+([0-9.eE+-]+)\\ngini\s*=\s*([0-9.eE+-]+)\\nsamples\s*=\s*(\d+)\\nvalue\s*=\s*\[(\d+),\s*(\d+)\]\\nclass\s*=\s*(\d)")regex");
    std::regex rg_leaf(R"regex(^\s*(\d+)\s+\[label="gini\s*=\s*([0-9.eE+-]+)\\nsamples\s*=\s*(\d+)\\nvalue\s*=\s*\[(\d+),\s*(\d+)\]\\nclass\s*=\s*(\d)")regex");
    std::regex rg_edge(R"regex(^\s*(\d+)\s*->\s*(\d+))regex");
    ...
    // Field map for feature names
    std::unordered_map<std::string, int> field_map = {
        {"Timestamp", 0}, {"ID", 1}, {"DLC", 2},
        {"D0", 3}, {"D1", 4}, {"D2", 5}, {"D3", 6}, {"D4", 7}, {"D5", 8}, {"D6", 9}, {"D7", 10}
    };

    // Function to parse feat like "t9:Timestamp" to index
    auto parse_feat = [&](const std::string& feat_str) -> int {
        size_t colon = feat_str.find(':');
        if (colon == std::string::npos) throw std::runtime_error("Bad feat format: " + feat_str);
        std::string t_str = feat_str.substr(0, colon);
        std::string field = feat_str.substr(colon + 1);
        if (t_str[0] != 't') throw std::runtime_error("Bad time prefix: " + t_str);
        int t = std::stoi(t_str.substr(1));
        if (t < 0 || t >= SEQ_LEN) throw std::runtime_error("Invalid time step: " + t_str);
        auto it = field_map.find(field);
        if (it == field_map.end()) throw std::runtime_error("Unknown field: " + field);
        return t * PER_FRAME + it->second;
    };
    ...
    while (std::getline(in, line)) {
        if (std::regex_search(line, m, rg_node)) {
            ...
            std::string feat_str = m[2];
            n.feat = parse_feat(feat_str);
            n.thr = std::stof(m[3]);
            // Ignore gini m[4], samples m[5]
            n.val0 = std::stoi(m[6]); n.val1 = std::stoi(m[7]);
            id2idx[id] = (int)tree.size(); tree.push_back(n);
            max_feat_idx = std::max(max_feat_idx, n.feat);
        } else if (std::regex_search(line, m, rg_leaf)) {
            int id = std::stoi(m[1]); Node n;
            // Ignore gini m[2], samples m[3]
...
int main(int argc, char** argv){
    try{
        ...
        if (live_iface.empty()){
            std::cerr << "Usage: sudo ./ids_live_labeled --live <can-iface>\n"
                         "Example: sudo ./ids_live_labeled --live can0\n";
            return 1;
        }
